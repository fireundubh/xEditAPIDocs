...
```pascal
procedure ExtraDatasByType(ABlock: TwbNifBlock; ABlockType: string; AList: TList);
procedure ExtraDatasByType(ABlock: TwbNifBlock; ABlockType: string; AList: TList; AInherited: Boolean);
```
...
The function can search for exact type matches or include inheritance. When inheritance checking is enabled, it includes all extra data blocks that are of the specified type or inherit from it.
...
begin
  ...
  try
    ...
    if Assigned(rootNode) then begin
      ...
      ExtraDatasByType(rootNode, 'NiStringExtraData', stringDataList, False);
